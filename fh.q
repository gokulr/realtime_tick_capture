/
 data structures used: dictionaries and operations on dictionaries (extract key/value, apply functions to dicts, etc.)
 concepts used:- ipc: socket handles, sync and async message passing
 adverbs (each , each-both , apply each-both (@'), projections
\
...
TPH:hopen TP                                                                    / connect to tickerplant on localhost and port 5000; and store handle in a global variable TPH
...
\
map_types:{[types;m]
 :(!). ((key m);types[key m]@'value m);                                        / cast string values to types; interesting stufff!!
 };
...
\
prs:{[m;c;tf]
 c upsert enlist value map_types[tf;m];                                         / parse message and append to relevant cache
 };
...
\
pubh:{[t;d]
 if[count d;(neg TPH)(`upd;t;d)];                                               / if there is data in d, publish (async) on table t
 };
...
\
pub:{[]
 pubh[`trade;flip TRADES];                                                      / publish trade and quote using pubh helpder function
 pubh[`quote;flip QUOTES];
 };
...
upd:{[msgs]
  ...
  pub[];                                                                        / pub all messages in caches
