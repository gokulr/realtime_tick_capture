...
// setup the process
										/ connect to tickerplant on localhost and port 5000; 
...
// quote fids!parse_types to extract
										/ setup sym,bid,ask,bsize,asize fid and cast mappings (QUOTE_FIDS global)
...
// return - nothing
prs:{[m;c;tf]						
  										/ parse message and append to relevant cache
 };
...
/ qparse									/ cast to correct types using QUOTE_FIDS and append to QUOTES
...
mparse:{[m]
 m:p m;										/ parse strings to key-value pairs (key is the fid, so fid-value pairs)
 										/ if any fid in trade specific fids (sym doesn't count) parse trades
 										/ if any fid in quote specific fids (sym doesn't count) parse quotes
 };
...
// return - nothing
pubh:{[t;d]
  										/ if there is data in d, publish (async) on table t
 };			

// publishes trade and quote events
// return - nothing
pub:{[]
  										/ publish trade and quote using pubh helpder function
...
// return - nothing
upd:{[msgs] 
  										/ parse each message using mparse function
  										/ pub all messages in caches
